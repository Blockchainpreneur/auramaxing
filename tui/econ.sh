#!/usr/bin/env bash
# AURAMXING launcher — starts daemon if needed, then Python TUI

DAEMON_PORT=57821
DIR="$(cd "$(dirname "$0")/.." && pwd)"
LOCK_FILE="/tmp/auramxing-tui.lock"
LOCAL_VERSION=$(cat "$DIR/VERSION" 2>/dev/null | tr -d '[:space:]')
VERSION_URL="https://raw.githubusercontent.com/Blockchainpreneur/AURAMXING/main/VERSION"
UPDATE_CMD="cd ~/auramxing && git pull && bash install.sh"
...
if [ -f "$LOCK_FILE" ]; then
  ...
  if [ -n "$OLD_PID" ] && kill -0 "$OLD_PID" 2>/dev/null; then
    echo "AURAMXING is already running (PID $OLD_PID)" >&2
    exit 0
  fi
  rm -f "$LOCK_FILE"
fi
...
exec python3 "$DIR/tui/auramxing.py"

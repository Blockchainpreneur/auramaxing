...
9. **SONNET AT MAXIMUM presets — 10x forced diligence (non-negotiable)** — Sonnet is only useful next to Fable under 10x forced diligence. EVERY worker prompt MUST include the word **"ultrathink"** (engages the worker's MAXIMUM extended-thinking budget — no exceptions) and run at maximum effort (never lower a worker's capability to save tokens — the savings come from the model tier, not from crippling the worker). The spec is HEAVY prompt-engineering: evidence contract (the return must QUOTE real run output — test/build/lint logs + file:line) and banned phrases ("should work", "I think", "done" without run output ⇒ instant REJECT). A PreToolUse guard (`ultramax-guard.mjs` normal mode) hard-blocks any sonnet/haiku spawn whose prompt is missing "ultrathink" or the ZERO-TOLERANCE frame — if blocked, re-issue the same spawn with both embedded.

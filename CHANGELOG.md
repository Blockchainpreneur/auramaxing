...
- **NO-EXCUSES:** greatness evidence containing a rationalization for stopping short — "good enough", "MVP-fine", "out of scope", "should work", "future work", TODO, placeholder, "for the purposes of this", "in a real implementation you would", "partial", "due to time" — is **auto-rejected** (tight regex; a real proof never trips it). Either finish to greatness or get the user to explicitly de-scope.
- **ADVERSARIAL-BEFORE-GREAT:** a `refineRequired` deliverable cannot close until the session ran an **independent critic** — a real `/review`·`/cso`·`/codex`·`/design-review` (or `codex review` in Bash) — OR carries a `ledger.mjs adversary <id> "<attacked·found·fixed>"` record. Self-certification is no longer greatness.
- **Distinct refinement rounds:** convergence now requires **≥2 DISTINCT** refinement deltas, not N copies of the same shallow note (anti-gaming). The Gate-3 message tells the model when its rounds aren't distinct.
- **New `ledger.mjs adversary <id> "<note>"`** records an inline critic pass. Router deliverable items now spell out the refine→adversary→great sequence; new `NO EXCUSES + ADVERSARIAL-BEFORE-GREAT` router directive + `[no-excuses + adversarial]` prompt-engine principle + ORCHESTRATION §0.0 Gate-3 doctrine.
- Regression: +8 cases; existing convergence fixtures updated to the stricter bar. Backward-compatible: non-`refineRequired` greatness unchanged; fail-open + bounded-nudge preserved. Eval **149→157/157**.
- **Hardened against its own adversarial review** (an independent critic subagent attacked the diff before ship — dogfooding the new bar): **C1** dropped `eslint`/bare-`codex` from adversarial detection (a linter is the Gate-1 verify, not an independent critic — it let the requirement be satisfied with zero second opinion; only `codex review`/`codex challenge` + the critic skills count now); **C2** a self-written `adversary` note must be ≥24 chars (a 1-char "." no longer clears it); **C3** refinement deltas must be ≥6 chars to count as distinct (anti-trivial-variation); **M1** excuse regex now catches "partial implementation", "needs polish", "not production-ready", "for the moment", "later iteration"; **M2** dropped bare "placeholder/TODO/stub" so honest "removed all TODOs" proof no longer false-positives. Honest limit: free-text checks (note/delta/excuse) are best-effort speed-bumps — the real teeth are the critic-skill path + the verify requirement + process forcing. Safety contract (fail-open / never-wedge / never-crash) was verified UPHELD by the review across all malformed inputs.
...
- **F5 — nudge `turnKey` keyed on the prompt's stable `uuid` (+session), not the line index.** The old key hashed `prompt[:160]` + line index, so (a) auto-compact rewriting the transcript reset the counter mid-task and (b) distinct prompts at the same index collided. Now prefers the transcript line's `uuid` (survives line shifts + compaction), content-hash fallback drops the index. Genuine-prompt detection is structural (`isToolResultCarrier`) instead of a `text.includes('tool_result')` substring.
- **F7 — atomic state writes (temp + rename).** `ledger.mjs`, the gatekeeper nudge/billion files, and the router's auto-ledger write now write-tmp-then-rename. A concurrent hook process can no longer read a half-written JSON → `JSON.parse` throw → silent fail-open.
- **F8 — greatness stamp cross-validated against the transcript.** `ledger.mjs great` evidence is free text, so a bare "ok" used to clear Gate 3. The stamp now also requires a real verification event (test/build/lint/typecheck runner or a verify Skill, non-failing) somewhere in the session transcript — a confident-sounding evidence string with no test run anywhere is a rubber stamp and keeps blocking. Fail-open on unreadable transcript.
- **L6 — handoff-aware ledger migration.** After auto-compact the new `session_id` orphaned the pre-compact ledger (session-scoped gates silently fail-opened across the handoff). PreCompact stamps the predecessor session (`ledger-lineage.json`); PostCompact runs `ledger.mjs migrate <from> <to>`, re-stamping the predecessor's still-OPEN items onto the successor (done items are history). Isolation-safe: only the explicit predecessor is adopted.
- **FIX E — per-phase ledger decomposition (both audits' top ask).** The router opened one generic ledger item; the 12-phase loop was never gate-forced. A fresh substantial task (complexity ≥50, no carried items) now decomposes into per-phase items (build → test+verify → audit → improve → ABSOLUTE GREATNESS GATE) so Gate 2 forces each phase to close. Growth is bounded: continuations append a single item, small tasks stay single-item — a multi-prompt session can't balloon.
- **Minor hygiene.** `context-threshold-monitor` 40%-handoff fire-once flag re-keyed on `session_id` (sanitized) instead of `process.ppid` (not a stable session identity → could double-fire/miss); ppid stays as fallback. `setup/CLAUDE.global.md` MCP roster corrected to the real set (context7 · shadcn · magicui · designlang · deepwiki · chrome-devtools + claude.ai connectors) — the reinstall template still listed the removed sentry/supabase/github/figma/n8n/sequential-thinking. Verified `taste.mjs` is NOT orphan (the eval suite tests it) — left intact.
- **Box decommission** (shipped in this release's first commits): the old Hetzner box IP was burned in git history; `cloud/lib.sh` `aura_resolve_host()` + new `cloud/beacon.sh` resolve the fleet target via the REX beacon (or `AURA_FLEET_HOST`), never a hardcoded IP.
- **Residuals (user-gated):** NLM auth is expired — the 3,507 dead-letter entries are auth-casualties (recoverable via `nlm-replay-buffer.mjs` once re-authed), so they were NOT archived/truncated; fix = sign into notebooklm.google.com → `nlm-cookie-sync.mjs` → `nlm-replay-buffer.mjs`. `.github/workflows/shellcheck.yml` still held back pending `gh auth refresh -s workflow` (PAT lacks the `workflow` scope).

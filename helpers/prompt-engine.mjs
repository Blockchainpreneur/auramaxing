...
try {
  ...
  structuredPrompt += '\n[refinement-convergence]: A working result is NOT a deliverable. Treat the work as a series of deliverables, each in an INFINITE refinement loop that ships ONLY at convergence — when a full refinement round (correctness · robustness/edge-cases · clarity · performance · security · design) yields ZERO material improvement AND you can prove, against an adversarial skeptic, that this IS the maximum-refinement version. If a critic could still improve it you have NOT converged — keep looping. Record each round (`ledger.mjs refine <id> "..."`); the gatekeeper blocks a refineRequired close below the minimum rounds. Stricter than 100/100: deliver only what survives maximum refinement.';
  structuredPrompt += '\n[no-excuses + adversarial]: The gatekeeper ENFORCES effort over excuses. NEVER stop on a rationalization — "good enough", "MVP-fine", "out of scope", "should work", "future work", TODO, placeholder, "for the purposes of this", "in a real implementation you would" are BANNED as a close and auto-rejected in greatness evidence; either finish to greatness or get the USER to explicitly de-scope. Do not manufacture reasons you cannot continue — exhaust research/retries/alternatives first. And you are NOT the judge of your own greatness: BEFORE claiming done on a substantial deliverable, run an INDEPENDENT critic that tries to BREAK it (/review, /cso, /codex, /design-review), FIX every finding, re-verify, and record it (`ledger.mjs adversary <id> "..."`). The bar is what survives a hostile expert review, not what merely runs.';

...
async function main() {
  ...
  if (fableWindow && !ultramax) {
    directives.push(`🔒 FABLE-ONLY WINDOW (active until 2026-06-23, then back to normal automatically): ALL work and ALL delegation run on Fable 5 EXCLUSIVELY — no Sonnet/Haiku workers anywhere (Agent/Task/Workflow). Spawns OMIT the model parameter (inherit claude-fable-5 + ultracode) or set model:"fable", and every spawned prompt includes "ultrathink". The aura-delegate Sonnet protocol is SUSPENDED during the window (its discipline — atomic specs, acceptance tests, evidence contracts — still applies to the Fable fleet). A PreToolUse guard hard-blocks any sonnet/haiku spawn while the window is active.`);
  }

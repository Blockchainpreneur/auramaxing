...
async function main() {
  ...
  // FABLE-ONLY WINDOW (user directive 2026-06-12): until the date in
  // ~/.auramaxing/fable-window.json, ALL delegation runs on Fable 5 — no Sonnet/Haiku
  // anywhere. Expires by itself (date compare); delete the file to end it early.
  ...
  try {
    ...
    fableWindow = !!fw.until && Date.now() < Date.parse(fw.until + 'T05:00:00Z'); // midnight Cancun (UTC-5)
  } catch {}
  ...
  const tier           = billion ? 'OPUS 4.8 · BILLION' : ultramax ? 'OPUS 4.8 · ULTRAMAX' : complexity < 30 ? 'HAIKU' : complexity < 65 ? 'SONNET' : 'OPUS';
  ...
  if (complexity >= 50 && primary.agents?.length) {
    ...
    if (complexity >= 50) {
      ...
      if (!ultramax && !fableWindow) {
        directives.push(`RETRIEVE-FIRST + DELEGATE (token-efficient orchestration): Before any edit, RETRIEVE the minimal high-signal context (Grep/Glob/Explore + memory recall) — never grep-and-read whole files when the index answers it. DELEGATION (apply the \`aura-delegate\` skill — the full strict Opus→Sonnet protocol): YOU own the decision boundaries — plan, spec, accept/reject, fuse, and the few CRUCIAL edits (stay token-efficient). Push BULK labor (drafts, broad research, exploration, mechanical edits) to cheaper Sonnet workers (Task tool / box fleet) under a STRICT harness: one atomic fully-specified sub-task each, shipped WITH its acceptance test; REJECT any worker output unless a deterministic gate passes (verify OUTCOMES not utterances). Never trust a Sonnet return as-is — gate it, or re-spec and re-run it. EVERY Sonnet sub-task MUST carry the 8 ZERO-TOLERANCE RULES verbatim + the Tier-2 micro-loop (scope+20x hypothesis → build → /qa → /review+/cso → improve → ABSOLUTE GREATNESS GATE, 3× YES with evidence) — paste them into the worker spec as a forcing frame so the cheaper model performs at its ceiling; a return missing the loop evidence is REJECTED on sight and re-spec'd with the loop made explicit. (Full text: ~/auramaxing/docs/ORCHESTRATION.md §0.0 / aura-delegate skill harness rule 8.) SONNET AT MAXIMUM — 10x FORCED DILIGENCE (non-negotiable): a Sonnet worker is only useful next to Opus 4.8 under 10x forced diligence, so EVERY Sonnet/Haiku worker runs at MAXIMUM capability presets: its prompt MUST include the word "ultrathink" (engages the worker\'s MAXIMUM extended-thinking budget — no exceptions, every worker) and maximum effort (never lower a worker\'s capability); the spec is HEAVY prompt-engineering: ONE atomic deliverable + its acceptance test + an EVIDENCE CONTRACT (the return must QUOTE real run output — test/build/lint logs + file:line — or it is auto-REJECTED) + BANNED phrases ("should work", "I think", "done" without run output ⇒ instant REJECT + re-spec). A PreToolUse guard hard-blocks any sonnet/haiku spawn whose prompt is missing "ultrathink" or the ZERO-TOLERANCE frame.`);
      }
    }
  } else {
  ...
  if (ultramax) {
    directives.unshift(`⚡ ULTRAMAX MODE — OPUS 4.8 EXCLUSIVE (NON-NEGOTIABLE): this entire task runs on Opus 4.8 ONLY, at MAXIMUM capability presets. NO other model anywhere — no Sonnet/Haiku workers, no \`model:"sonnet"\`/\`"haiku"\` on any Agent/Task/Workflow call, no box fleet (orchestra.sh / nightly / SSH to the box). DELEGATION IS ALLOWED — but ONLY to Opus 4.8 multi-agents at max settings: (1) every Agent/Task spawn OMITS the model parameter (inherits the Opus 4.8 session default: claude-opus-4-8 + effortLevel xhigh) or sets \`model:"opus"\`; (2) every spawned agent prompt MUST include the word "ultrathink" so the fleet agent engages its MAXIMUM extended-thinking budget — no exceptions, every spawn; (3) Workflow scripts must NOT set any non-Opus \`model:\` override on agent() or meta.phases; (4) MAXIMUM presets EVERYWHERE, no exceptions — YOU (the main agent) engage extended thinking (ultrathink) on every phase of the loop, and main + every fleet agent run at maximum effort (the xhigh session default — never lower it); (5) ULTRAMAX = maximum scope of capability and work: every applicable phase of the Absolute Perfection Loop becomes a visible /goal task (TaskCreate per phase), closed only with gate evidence. YOU (Opus 4.8) remain the orchestrator and reviewer: the aura-delegate Sonnet protocol is SUSPENDED, but its DISCIPLINE is not — every Opus sub-task is still one atomic fully-specified spec shipped WITH its acceptance test, carries the 8 ZERO-TOLERANCE RULES + the Tier-2 micro-loop verbatim, and its return is REJECTED unless a deterministic gate passes (verify OUTCOMES, not utterances). Keep the FULL AURAMAXING structure intact — visible goal-loop (TaskCreate step-list), phased-excellence loop (audit→investigate→plan→execute→verify, 100/100 with evidence), anti-laziness, evidence gates, adversarial self-verify. A PreToolUse guard hard-blocks any non-Opus spawn, any spawn prompt missing "ultrathink", and any Workflow model override; if blocked, re-issue the SAME call corrected per the block message. Use the Opus 4.8 fleet to fan out, adversarially verify, and synthesize — exhaustive correctness over token cost. Kill-switch: AURA_ULTRAMAX_OFF=1.`);
  }
  // FABLE-ONLY WINDOW directive (no-op under ultramax/billion — already Fable-only there).
  if (fableWindow && !ultramax) {
    directives.push(`🔒 OPUS-ONLY WINDOW (active until 2026-06-23, then back to normal automatically): ALL work and ALL delegation run on Opus 4.8 EXCLUSIVELY — no Sonnet/Haiku workers anywhere (Agent/Task/Workflow). Spawns OMIT the model parameter (inherit claude-opus-4-8 + xhigh) or set model:"opus", and every spawned prompt includes "ultrathink". The aura-delegate Sonnet protocol is SUSPENDED during the window (its discipline — atomic specs, acceptance tests, evidence contracts — still applies to the Opus 4.8 fleet). A PreToolUse guard hard-blocks any sonnet/haiku spawn while the window is active.`);
  }

...
import { findPython, findNlmArgs, pythonEnv } from "./find-bin.mjs";

const HOME = homedir();
const NLM = findNlmArgs();
if (!NLM) { process.stderr.write('[nlm] NotebookLM CLI not installed. Skipping.\n'); }
...
function nlm(args) {
  try {
    return execFileSync(NLM.bin, [...NLM.args, ...args], { encoding: 'utf8', timeout: 30000 }).trim();
  } catch (e) {

...
// Single-quote a string for safe shell interpolation (neutralizes ", `, $(), etc.)
function shq(s) {
  ...
  case 'synthesize': {
    ...
    if (synthType === 'learnings') {
      ...
      if (sourceData.length > 10) {
        const result = nlm(`ask ${shq(`Synthesize these tool learnings into exactly 5 concise rules. Each rule should be actionable. Format: numbered list. Learnings:\n${sourceData.slice(0, 1500)}`)}`);
        console.log(result);
      } else {

  ...
  case 'synthesize': {
    ...
    if (synthType === 'learnings') {
      ...
      if (sourceData.length > 10) {
        const question = `Synthesize these tool learnings into exactly 5 concise rules. Each rule should be actionable. Format: numbered list. Learnings:\n${sourceData.slice(0, 1500)}`.replace(/"/g, '\\"');
        const result = nlm(`ask "${question}"`);
        console.log(result);
      } else {

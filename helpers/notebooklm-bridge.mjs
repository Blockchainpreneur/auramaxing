...
switch (command) {
  ...
  case 'compress-memory': {
    ...
    const briefingPrompt = `Compress these session logs into a 3-sentence briefing. Include: project, key decisions, current status, next actions: ${raw.slice(0, 2000)}`;
    const compressed = nlm(`ask '${briefingPrompt.replace(/'/g, `'\\''`)}'`);

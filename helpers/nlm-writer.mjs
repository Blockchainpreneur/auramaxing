    ...
    try {
      if (method === 'source-research') {
        ...
        if (typeof entry.payload === 'string' && /^https?:\/\//.test(entry.payload.trim())) {
          nlm(`source add-research '${entry.payload.trim().replace(/'/g, `'\\''`)}' --title '${String(title).replace(/'/g, `'\\''`)}'`, { timeout: 45000 });
        } else {
...
export async function flush({ limit = Infinity } = {}) {
  ...
  // Requeue any entries beyond the limit so they aren't dropped (BUFFER/RETRY already truncated)
